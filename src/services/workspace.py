...
- **NEVER** use built-in `read_file`, `list_files`, or `search` tools for workspace \
content. Use `ws__*` MCP tools exclusively.
...
as a workspace file.
- For binary files (xlsx, docx, pptx, pdf) generated by a skill: first write the file \
with the skill's `write_file` tool, then call `ws__publish_workspace_file` to transfer \
it into the workspace. Do NOT skip the publish step.
